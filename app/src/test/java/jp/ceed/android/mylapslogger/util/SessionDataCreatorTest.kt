...
internal class SessionDataCreatorTest : Spek({
    ...
    val sessionDataCreator = SessionDataCreator(
        context = mockk(relaxed = true)
    )

    describe("ラップリストの生成") {
        context("セッション番号指定の場合") {
            val lapList: List<PracticeResultsItem> = sessionDataCreator.createLapList(sessionResponse, 1)
            it("要素の数が正しい"){
        ...
        context("セッション番号未指定の場合") {
            val lapList: List<PracticeResultsItem> = sessionDataCreator.createLapList(sessionResponse, 0)
            it("要素の数が正しい"){
