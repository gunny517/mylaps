...
interface PracticeTrackDao {
    ...
    @Query(
        "SELECT p.id, p.track_id, t.name, p.lap_count, p.best_lap, " +
                "p.start_time, p.end_time, p.display_time, p.total_training_time " +
    ...
    @Query(
        "SELECT t.id, t.name, t.length, sum(p.lap_count) AS total_lap_count, " +
                "(t.length * sum(p.lap_count)) AS distance, count(p.id) AS training_count " +
